...
whole-report engagement assessment, or the whole-report team-learning analysis. The MCP server
validates the submission through the generation API, resolves every citation to its indexed-turn line
range, and atomic-writes the patched report.
...
file with the three synthesize slots set to `null` before any synthesis pass runs; the write tools
...
- **Chain-only parse.** The submission's structure is validated first: non-empty strings, controlled
  ...
  citations (`write_engagement`, `write_team_learning`) require `project_key`; per-project citations
  (`write_project_summary`) omit it.
