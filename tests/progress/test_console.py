...
def test_generate_render_shows_completed_phase_duration() -> None:
    stream = io.StringIO()
    console = Console(file=stream, force_terminal=False, width=100)
    with LiveConsoleReporter(console=console) as reporter:
        reporter.emit(PhaseStarted(at=1.0, phase_id="rendering", label="rendering"))
        reporter.emit(PhaseFinished(at=3.5, phase_id="rendering", status="success"))
...
def test_live_renderable_refreshes_elapsed_without_new_event(
    ...
    reporter.emit(PhaseStarted(at=0.0, phase_id="rendering", label="rendering"))
