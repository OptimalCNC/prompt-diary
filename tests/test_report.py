...
def test_build_report_prompt_contains_prompt_contract(tmp_path: Path) -> None:
    ...
    prompt = build_report_prompt(workspace)
...
def test_empty_fallback_report_satisfies_validation_without_index_claims(tmp_path: Path) -> None:
    ...
    report_path = write_empty_fallback_report(workspace)
...
def test_command_report_writer_runs_command_in_workspace(tmp_path: Path) -> None:
    ...
    assert (workspace / "prompt.txt").read_text(encoding="utf-8") == "prompt\n"
...
def test_command_report_writer_reports_start_failure(tmp_path: Path) -> None:
    ...
    with pytest.raises(ReportWriterError, match="could not start"):
        writer.write_report(workspace_path=workspace, prompt="prompt")
...
def test_fallback_writers_generate_partial_note_and_validate(tmp_path: Path) -> None:
    ...
    deterministic_path = write_deterministic_report(workspace)
    ...
    assert validate_report(workspace).ok
...
def test_validate_report_rejects_citation_outside_index_span(tmp_path: Path) -> None:
    workspace = _workspace_fixture(tmp_path)
    _write_claim_report(workspace, line_span="1-5")
...
def test_validate_report_rejects_partial_report_without_note(tmp_path: Path) -> None:
    workspace = _workspace_fixture(tmp_path)
    _set_metadata_status(workspace, "partial")
    _write_report_lines(workspace, _valid_empty_report_lines(status="partial"))
...
def test_validate_report_rejects_mixed_fallback_and_claim_bullets(tmp_path: Path) -> None:
    ...
    lines = _valid_empty_report_lines()
...
def test_validate_report_surfaces_metadata_load_errors(tmp_path: Path) -> None:
    ...
    missing_validation = validate_report(missing_metadata_workspace)
    ...
    invalid_validation = validate_report(invalid_workspace)
    ...
    scalar_validation = validate_report(scalar_workspace)
    ...
    missing_object_validation = validate_report(missing_object_workspace)
    assert any(
    ...
    missing_string_validation = validate_report(missing_string_workspace)
    assert any(
...
def test_validate_report_surfaces_project_and_index_shape_errors(tmp_path: Path) -> None:
    ...
    write_empty_fallback_report(missing_project_label_workspace)

    project_validation = validate_report(missing_project_label_workspace)
    assert any(
    ...
    index_path.write_text("{\n", encoding="utf-8")
    write_empty_fallback_report(invalid_index_workspace)

    invalid_index_validation = validate_report(invalid_index_workspace)
    ...
    write_empty_fallback_report(scalar_index_workspace)

    scalar_index_validation = validate_report(scalar_index_workspace)
...
def test_validate_report_rejects_index_field_errors(tmp_path: Path) -> None:
    ...
    _write_jsonl(index_path, [row])
    write_empty_fallback_report(missing_string_workspace)

    string_validation = validate_report(missing_string_workspace)
    assert any(
    ...
    write_empty_fallback_report(missing_int_workspace)

    int_validation = validate_report(missing_int_workspace)
    assert any(
    ...
    write_empty_fallback_report(invalid_path_workspace)

    path_validation = validate_report(invalid_path_workspace)
...
def test_validate_report_rejects_session_index_boundary_errors(tmp_path: Path) -> None:
    ...
    _write_jsonl(index_path, [row])
    write_empty_fallback_report(nonpositive_workspace)

    nonpositive_validation = validate_report(nonpositive_workspace)
    assert any(
    ...
    write_empty_fallback_report(missing_file_workspace)

    missing_file_validation = validate_report(missing_file_workspace)
    ...
    write_empty_fallback_report(exceeding_workspace)

    exceeding_validation = validate_report(exceeding_workspace)
    assert any(
...
def _write_claim_report(workspace: Path, *, line_span: str) -> None:
...
def test_validate_report_handles_invalid_turns_field(tmp_path: Path) -> None:
    ...
    _write_jsonl(index_path, [row])
    write_empty_fallback_report(not_list_workspace)
    not_list_validation = validate_report(not_list_workspace)
    ...
    write_empty_fallback_report(non_dict_workspace)
    non_dict_validation = validate_report(non_dict_workspace)
    ...
    write_empty_fallback_report(bad_fields_workspace)
    bad_fields_validation = validate_report(bad_fields_workspace)
