...
_TITLE_DATE = re.compile(r"\b\d{4}-\d{2}-\d{2}\b")
...
@dataclass(frozen=True)
class ReportTitle:
    """The whole-report headline, generated by daily synthesis and cited like other claims."""
...
@dataclass(frozen=True)
class ParsedReportTitle:
    """A successful parse of the whole-report title submission."""
...
ReportTitleParseResult: TypeAlias = ParsedReportTitle | InvalidDailyReportInput
...
def parse_report_title(title: dict[str, Any]) -> ReportTitleParseResult:
    """Parse the whole-report title submission into a typed value or structured errors."""
    errors: list[DailyReportWriteError] = []
    parsed = _parse_report_title(title, errors)
    if errors:
        return InvalidDailyReportInput(tuple(errors))
    return ParsedReportTitle(parsed)
...
def _parse_report_title(raw: dict[str, Any], errors: list[DailyReportWriteError]) -> ReportTitle:
    mapping = _as_mapping(raw)
    return ReportTitle(
        text=_parse_title_text(mapping.get("text"), errors, path="title.text"),
        citations=_parse_citations(
            mapping.get("citations"), errors, path="title.citations", require_project_key=True
        ),
    )
...
def _parse_title_text(value: object, errors: list[DailyReportWriteError], *, path: str) -> str:
    text = _parse_nonempty(value, errors, path=path)
    if not text.strip():
        return text
    if "\n" in text or "\r" in text:
        errors.append(DailyReportWriteError(path, _single_line_title_message(path), _TITLE_HINT))
    if len(text) > _MAX_TITLE_LENGTH:
        errors.append(DailyReportWriteError(path, _long_title_message(path), _TITLE_HINT))
    normalized = " ".join(text.casefold().split())
    if _TITLE_DATE.search(text):
        errors.append(DailyReportWriteError(path, _dated_title_message(path), _TITLE_HINT))
    if normalized in _GENERIC_TITLE_PHRASES or "prompt diary report" in normalized:
        errors.append(DailyReportWriteError(path, _generic_title_message(path), _TITLE_HINT))
    return text
...
def _single_line_title_message(path: str) -> str:
    return f"{path} must be a single-line report title"


def _long_title_message(path: str) -> str:
    return f"{path} must be at most {_MAX_TITLE_LENGTH} characters"


def _dated_title_message(path: str) -> str:
    return f"{path} must not include the report date"


def _generic_title_message(path: str) -> str:
    return f"{path} must name the day's actual work, not a generic report label"


_NONEMPTY_HINT = "provide a concise non-empty string"
_TITLE_HINT = "write a concise, specific title without date, Markdown, or generic report wording"
