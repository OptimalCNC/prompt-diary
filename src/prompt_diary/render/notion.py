...
def render_workspace_report_to_notion(
    ...
    *,
    ...
    try:
        ...
        secret, database_id = (
            credentials if credentials is not None else resolve_notion_credentials()
        )
        ...
        except Exception as exc:  # noqa: BLE001 - scrub the token from ANY publish failure
            # The token is in scope here, so scrub it from the surfaced message — structured errors
            # and unexpected SDK failures alike. Capture the redacted text and raise *below*,
            # outside this handler, rather than `from exc`: that keeps the raw, possibly
            # token-bearing cause out of the new error's __cause__/__context__, where a traceback
            # or an exc_info logger could otherwise surface it. (The token itself stays wrapped in
            # ``secret`` — never a bare str local — so even a locals-capturing traceback renders it
            # as ``***``. The render step above has no token in scope, so it needs none.)
            redacted = _redact(
